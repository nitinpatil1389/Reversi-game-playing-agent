...
public class AlphaBeta {
	...
	public static OutputValueObj adversarialSearchAlgorithms(Integer taskNo, char player, Integer cutOffDepth, char[][] stateMatrix) {
		...
		//print only for mini-max and alpha-beta algorithms, create traverse log
		traverseLog.add("Node,Depth,Value,Alpha,Beta");
		
		//get the child nodes of root
		StateNode rootNode=new StateNode("root", null, 0, stateMatrix);
		LinkedList<StateNode> childrenNodes =  Operators(game, rootNode, cutOffDepth, player, oppPlayer);
		
		//check if game is over before checking next move
		if(childrenNodes==null){
			int evalValue = Eval(game, stateMatrix);
			
			//traverseLog.add("root,0,-Infinity,-Infinity,Infinity");
			traverseLog.add("root,0,"+evalValue+",-Infinity,Infinity");
		...
		int value=Integer.MIN_VALUE, temp;
		...
		for(int op=0; op<childrenNodes.size(); op++){ 
			...
			traverseLog.add(curChildNode.getParent().getMove()+","+curChildNode.getParent().getDepth()+","+((value==Integer.MIN_VALUE)?"-Infinity":value)+","+((alpha==Integer.MIN_VALUE)?"-Infinity":alpha)+","+((beta==Integer.MAX_VALUE)?"Infinity":beta));
			
			temp = Math.max(value, minValue(game, curChildNode, cutOffDepth, alpha, beta));
			...
			if(value >= beta)//mostly unreachable
				break;
		...
		//traverse log
		traverseLog.add(rootNode.getMove()+","+rootNode.getDepth()+","+((value==Integer.MIN_VALUE)?"-Infinity":value)+","+((alpha==Integer.MIN_VALUE)?"-Infinity":alpha)+","+((beta==Integer.MAX_VALUE)?"Infinity":beta));
	...
	private static int minValue(final Game game, StateNode state, int cutOffDepth, int alpha, int beta) {
		...
		if(childrenStateNodes==null){
			...
			//traverse log
			traverseLog.add(state.getMove()+","+state.getDepth()+","+evalValue+","+((alpha==Integer.MIN_VALUE)?"-Infinity":alpha)+","+((beta==Integer.MAX_VALUE)?"Infinity":beta));
		...
		for(int op=0; op<childrenStateNodes.size(); op++){
			...
			//traverse log
			traverseLog.add(state.getMove()+","+state.getDepth()+","+((value==Integer.MAX_VALUE)?"Infinity":value)+","+((alpha==Integer.MIN_VALUE)?"-Infinity":alpha)+","+((beta==Integer.MAX_VALUE)?"Infinity":beta));
			
			//value = min(value, maxValue());
			value = Math.min(value, maxValue(game, curChildNode, cutOffDepth, alpha, beta));
	...
	public static int maxValue(final Game game, StateNode state, int cutOffDepth, int alpha, int beta){
		//determine the children nodes, returns null if cutOff depth is reached
		...
		for(int op=0; op<childrenStateNodes.size(); op++){
			...
			//traverse log
			traverseLog.add(state.getMove()+","+state.getDepth()+","+((value==Integer.MIN_VALUE)?"-Infinity":value)+","+((alpha==Integer.MIN_VALUE)?"-Infinity":alpha)+","+((beta==Integer.MAX_VALUE)?"Infinity":beta));
			
			//value = max(value, minValue())
			value = Math.max(value, minValue(game, curChildNode, cutOffDepth, alpha, beta));
